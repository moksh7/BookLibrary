...
def add_book(request):
    ...
    if request.user.is_superuser:
        if request.method=='POST':
            ...
            price = request.POST.get('price')
            rating = request.POST.get('rating')
            bookobj = Book.objects.filter(title=title,category=category)
            if bookobj:
                ...
                Book.objects.create(title=title, author=author, desc=desc, category=category, availability=num, price=price, rating=rating)
